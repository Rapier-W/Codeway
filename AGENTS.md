...
同路行是手机优先响应式 Web/PWA + 独立后端的用户同路匹配服务。MVP 面向校园及周边试点，支持行程匹配、聊天、双向确认、费用确认、争议处理、安全反馈和评价；不做司机端、资金托管、在线支付、车辆实时定位或自动行车守护。
...
- 客户端：Vue 3 + Vite + TypeScript + Vant + Pinia + PWA。
...
- 第三方打车：适配器 + 手动降级。
- 认证：手机号 + 七牛云短信验证码，HttpOnly/Secure/SameSite=Lax Cookie 会话。
- 部署：单台七牛云 VM 使用 Docker Compose 运行 Nginx、Web 静态文件、NestJS API 和 PostgreSQL。
- 地图：高德 JavaScript SDK + 高德 Web 服务 API；前后端 Key 分离。
