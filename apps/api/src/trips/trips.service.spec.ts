...
describe('TripsService', () => {
  ...
  it('rejects reuse of a join idempotency key by another user or trip', async () => {
    tx.user.findUnique.mockResolvedValue({ id: 'u3', phoneVerified: true });
    tx.trip.findUnique.mockResolvedValue({ id: 't2', capacity: 4, status: 'RECRUITING', femaleOnly: false, members: [{ memberCount: 1 }] });
    tx.tripMember.findUnique.mockResolvedValue({ id: 'm1', tripId: 't1', userId: 'u2', joinRequestKey: 'req-used', memberCount: 1 });

    await expect(service.join('u3', 't2', { memberCount: 1 } as any, 'req-used')).rejects.toBeInstanceOf(ConflictException);
    expect(tx.tripMember.create).not.toHaveBeenCalled();
  });

  it('creates the creator membership with one occupied seat', async () => {
    tx.user.findUnique.mockResolvedValue({ id: 'u1', phoneVerified: true });
    tx.trip.create.mockResolvedValue({ id: 't1', capacity: 4, status: 'RECRUITING', members: [{ userId: 'u1', role: 'CREATOR', memberCount: 1 }] });

    await service.create('u1', {
      origin: 'A', destination: 'B', departTime: new Date(Date.now() + 3600000).toISOString(), capacity: 4,
    } as any);

    expect(tx.trip.create).toHaveBeenCalledWith(expect.objectContaining({
