...
export class FarePlanService {
  ...
  async getFarePlan(tripId: string, userId: string) {
    const trip = await this.prisma.trip.findUnique({ where: { id: tripId }, include: { members: true } });
    if (!trip) throw new NotFoundException('TRIP_NOT_FOUND');
    if (!trip.members.some((m: any) => m.userId === userId)) throw new ForbiddenException('TRIP_MEMBER_REQUIRED');
    const revision = await this.prisma.farePlanRevision.findFirst({ where: { tripId, status: { not: 'SUPERSEDED' } }, orderBy: { sequence: 'desc' }, include: { confirmations: true } });
    const request = await this.prisma.farePlanChangeRequest.findFirst({ where: { tripId, status: 'PENDING' }, include: { decisions: true } });
  ...
  async createChangeRequest(tripId: string, userId: string, dto: CreateFarePlanChangeRequestDto, requestKey: string) {
    ...
    return this.tx(async c => {
      ...
      if (existing) {
        if (existing.tripId !== tripId || existing.requestedBy !== userId || normalizeFarePlanBody({ proposedPlan: existing.proposedPlan, reason: existing.reason }) !== normalizeFarePlanBody({ proposedPlan: normalized, reason: dto.reason ?? null })) throw new ConflictException('IDEMPOTENCY_KEY_REUSED');
        return existing;
      }
  ...
  async decideChangeRequest(requestId: string, userId: string, dto: FarePlanDecisionDto, idempotencyKey: string) {
    ...
    return this.tx(async c => {
      ...
      const trip = await this.lockTrip(c, req.tripId);
      const decision = req.decisions.find((d: any) => d.userId === userId);
      ...
      if (idem) { if (idem.requestId !== requestId || idem.userId !== userId || idem.decision !== dto.decision) throw new ConflictException('IDEMPOTENCY_KEY_REUSED'); return idem; }
      const now = new Date();
      ...
      if (req.status !== 'PENDING') return decision;
      ...
      await c.farePlanRevision.create({ data: { tripId: req.tripId, sequence: base.sequence + 1, plan: req.proposedPlan, status: 'PENDING_CONFIRMATION' } });
  ...
  async confirmRevision(tripId: string, revisionId: string, userId: string, idempotencyKey: string) {
    ...
    return this.tx(async c => {
      ...
      const confirmation = await c.farePlanConfirmation.upsert({ where: { revisionId_userId: { revisionId, userId } }, create: { revisionId, userId, status: 'CONFIRMED', confirmedAt: new Date(), idempotencyKey }, update: { status: 'CONFIRMED', confirmedAt: new Date(), idempotencyKey } });
      const memberIds = trip.members.map((m: any) => m.userId);
      const confirmed = [...revision.confirmations.filter((x: any) => x.status === 'CONFIRMED').map((x: any) => x.userId), userId];
      if (memberIds.every((id: string) => confirmed.includes(id))) {
        const now = new Date();
