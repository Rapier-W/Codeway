...
// 真实 HTTP 默认走短信验证码；仅显式开启时才允许本地 dev-login 降级。
const isHttpMode = import.meta.env.VITE_API_MODE === 'http'
const isDevLogin = isDevLoginEnabled(import.meta.env.VITE_API_MODE, import.meta.env.VITE_ENABLE_DEV_LOGIN)
...
async function login() {
  ...
  busy.value = true
  try {
    if (isDevLogin) await session.devLogin(phone.value)
