...
<template><main class="page"><div class="page-heading"><div><div class="eyebrow">今天想去哪里</div><h1>发现同行</h1></div><RouterLink class="icon-action" to="/trips/create" aria-label="发布行程">＋</RouterLink></div><div class="toolbar"><button class="secondary-button" @click="showFilter = true">筛选</button><select v-model="sort" aria-label="排序"><option value="time">出发时间</option><option value="spaces">空位优先</option></select></div><p class="muted">推荐仅依据出发时间、实名验证、历史履约、剩余空位与路线匹配等客观因素，不构成任何信用或安全担保。</p><AsyncState :loading="store.loading" :error="store.error" :empty="!store.loading && !store.error && items.length === 0" @retry="store.load()" /><section v-if="!store.loading && !store.error && items.length" class="trip-list"><TripCard v-for="trip in items" :key="trip.id" :trip="trip" @open="router.push(`/trips/${$event}`)" /></section><FilterSheet v-if="showFilter" v-model="filters" @close="showFilter = false" /></main></template>
